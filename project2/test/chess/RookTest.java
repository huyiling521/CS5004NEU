...
class RookTest {
    ...
    @BeforeEach
    void setUp() {
        black = new Rook(7,7,Color.BLACK);
        white = new Rook();
    }
    ...
    @Test
    void testCanKill() {
        assertTrue(black.canKill(new Rook(6,7, Color.WHITE)));
        assertFalse(black.canKill(white));
        IllegalArgumentException thrown = assertThrows(IllegalArgumentException.class, () -> {
            black.canKill(new Rook(4, 7, Color.BLACK));
        });
        assertEquals("The target piece is of the same color!", thrown.getMessage());

        assertFalse(white.canKill(black));
        assertTrue(white.canKill(new Rook(0, 1, Color.BLACK)));
        thrown = assertThrows(IllegalArgumentException.class, () -> {
            white.canKill(new Rook(0, 0, Color.BLACK));
        });
        assertEquals("The target position is the piece itself!", thrown.getMessage());
    }
}